...
class Facebook_Btn extends Component {
    ...
    login = () => {
        ...
        database.ref('/users/' + that.state.passtext + '/' + user.uid + '/').set(that.props.updateduser.Loged_in_user);
        // //pushing user data in firebase database

        // //hiding fb btn
        that.props.changing_fb_btn_view(true);
        // changing route
        if(that.state.passtext === 'login as Student') {
            history.push('/dashboard_Student');
        }
        if(that.state.passtext === 'login as Company') {
            history.push('/dashboard_Company');
        }
        if(that.state.passtext === 'login as Admin') {
    ...
    render() {
        if(this.props.updateduser.user_is_logged_in === false) {
            return(
        <RaisedButton label={this.state.passtext} primary={true} onClick={this.login} style={{fontSize: '40px', width: '180px',}} />
